...
const GeneralInformation: React.FC<TourSectionProps> = (props: TourSectionProps) =>{
  ...
  return (
    <Box p="lg" className="bg-gray-100">
      <Text size="md" fw={700}>Lise: <Text span>{ props.tour.highschool_name }</Text></Text>
      <Text size="md" fw={700}>Tarih ve Saat: <Text span>{dateString} {date.format("HH:mm")}-{dateEnd.format("HH:mm")}</Text></Text>
