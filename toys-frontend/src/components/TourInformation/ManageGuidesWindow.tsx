...
const ManageGuidesWindow: React.FC<ManageGuidesWindowProps> = ({
  ...
  const fetchGuides = async (type: "GUIDE" | "TRAINEE") => {
    ...
    try {
      ...
      if (data.length === 0) {
        setError(type === "GUIDE" ? "Mevcut rehber yok." : "Mevcut amatör rehber yok.");
        setLoading(false);
        return;
      }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Check if the current user is in the selected guides
      ...
      if (isCurrentUserSelected) {
        ...
        // Invite other guides and trainees
                  ...
                  ? guides.map((guide) => (
                      ...
                      >
                        <Group justify="space-between" w="100%">
                          <Text fw={500}>
                            {guide.name} {guide.id === userContext.user.id && "(Siz)"}
                          </Text>
                        </Group>
                      </Button>
                  ))
